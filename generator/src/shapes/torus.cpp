...
void torus_generator::generate(int argc, char **argv) const {
    ...
    } catch (const std::exception&) { throw InvalidArgumentsException("Center radius is not valid!"); }
    if(center_radius <= 0) 
    ...
    } catch (const std::exception&) { throw InvalidArgumentsException("Inner radius is not valid!"); }
    if(inner_radius <= 0) 
    ...
    } catch (const std::exception&) { throw InvalidArgumentsException("Number of slices is not valid!"); }
    if(n_slices < 3)
    ...
    } catch (const std::exception&) { throw InvalidArgumentsException("Number of sections is not valid!"); }
    if(n_sections < 3)
